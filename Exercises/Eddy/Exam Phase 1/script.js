...
function add (date, Quantity, key) {
	...
	if (dates.indexOf (key) !== -1) {
		var operator = values[dates.indexOf (key)];
		flag = true;
	}
	...
	} else if (!flag && isType (date, Date, 'date') && isType (Quantity, Number, 'number') && (Quantity % 1 === 0) && Quantity !== Infinity) {
		return date;
	} else {
		return false;
	}
